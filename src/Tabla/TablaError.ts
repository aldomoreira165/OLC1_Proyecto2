...
export class TablaError {
    ...
    /*static graficarTabla(): string {
        let cadena: string;
        for (let index = 0; index < this.arregloErrores.length; index++) {
            cadena += `<tr><td>${this.arregloErrores[index].id}</td><td>${this.arregloErrores[index].tipo}</td><td>${this.arregloErrores[index].tipoVariable}</td><td>${this.arregloErrores[index].linea}</td><td>${this.arregloErrores[index].columna}</td></tr>\n`;
        }
        return cadena;
    }*/
}